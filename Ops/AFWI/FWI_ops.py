...
def load_fwi(path):
    fwi = load(name="AFWI", sources=[path+'/Torch_Fwi.cpp', path+'/Parameter.cpp', path+'/libCUFD.cu', path+'/propagate.cu', path+'/image_condition.cu',
...
class FWI(torch.nn.Module):
    def __init__(self, Vp, Stf, opt, Mask=None, Vp_bounds=None, vp_max=None):
        ...
        self.Bounds = {}
        Vp_pad= ft.padding(Vp, self.nz_orig, self.nx_orig, self.nz, self.nx, self.nPml, self.nPad)
...
class FWI_obscalc(torch.nn.Module):
    def __init__(self, Vp, Stf, opt, para_fname):
        ...
        Vp = ft.padding(Vp, opt['nz_orig'], opt['nx_orig'], opt['nz'], opt['nx'], opt['nPml'], opt['nPad'])
        self.Vp = Vp
        self.Stf = Stf
