...
extern "C" void cufd(float *misfit, float *grad_Vp, float *grad_stf, const float *Vp,
  ...
  float *fVp;
  fVp = (float *)malloc(nz * nx * sizeof(float));
  for (int i = 0; i < nz; i++) {
    for (int j = 0; j < nx; j++) {
      fVp[j * nz + i] = Vp[i * nx + j];
    }
  }
  ...
  CHECK(cudaMalloc((void **)&d_un, nz * nx * sizeof(float)));
  CHECK(cudaMalloc((void **)&d_up, nz * nx * sizeof(float)));
  CHECK(cudaMalloc((void **)&d_uc, nz * nx * sizeof(float)));
  CHECK(cudaMalloc((void **)&d_phiz, nz * nx * sizeof(float)));
  CHECK(cudaMalloc((void **)&d_phix, nz * nx * sizeof(float)));
  CHECK(cudaMalloc((void **)&d_un_adj, nz * nx * sizeof(float)));
  CHECK(cudaMalloc((void **)&d_up_adj, nz * nx * sizeof(float)));
  CHECK(cudaMalloc((void **)&d_uc_adj, nz * nx * sizeof(float)));
  ...
  for (int iShot = 0; iShot < group_size; iShot++) {
    ...
    intialArrayGPU<<<blocks, threads>>>(d_up, nz, nx, 0.0);
    intialArrayGPU<<<blocks, threads>>>(d_uc, nz, nx, 0.0);
    intialArrayGPU<<<blocks, threads>>>(d_un, nz, nx, 0.0);
    intialArrayGPU<<<blocks, threads>>>(d_phiz, nz, nx, 0.0);
    intialArrayGPU<<<blocks, threads>>>(d_phix, nz, nx, 0.0);
    intialArrayGPU<<<blocks, threads>>>(d_up_adj, nz, nx, 0.0);
    intialArrayGPU<<<blocks, threads>>>(d_uc_adj, nz, nx, 0.0);
    intialArrayGPU<<<blocks, threads>>>(d_un_adj, nz, nx, 0.0);
    ...
    for (int it = 0; it < nSteps - 1; it++) {
      ...
      #ifdef DEBUG
      if (it == 800 && iShot == 0) {
          CHECK(cudaMemcpy(h_snap, d_un, nz * nx * sizeof(float), cudaMemcpyDeviceToHost));
          fileBinWrite(h_snap, nz * nx, "SnapGPU_for__.bin");
      ...
      propagate<<<blocks, threads>>>(
                d_un, d_uc, d_up, d_phiz, d_phix,
                model.d_Vp, cpml.d_sigma_z, cpml.d_sigma_x, nz, nx, dt, dz, dx, nPml, nPad, true);

      // printf("source = %f, z_loc = %d, x_loc = %d\n", src_rec.vec_source.at(iShot)[it], src_rec.vec_z_src.at(iShot), src_rec.vec_x_src.at(iShot));

      add_source<<<1, threads>>>(d_un, src_rec.vec_source.at(iShot)[it],
                                 nz, true, src_rec.vec_z_src.at(iShot),
                                 src_rec.vec_x_src.at(iShot), dt, d_gauss_amp);
      exchange_wavefield<<<blocks, threads>>>(d_un, d_uc, d_up, nz);

      recording<<<(nrec + 31) / 32, 32>>>(
            d_uc, nz, d_data, iShot, it + 1, nSteps, nrec,
    ...
    if (para.withAdj()) {
      ...
      propagate<<<blocks, threads>>>(
          d_un_adj, d_uc_adj, d_up_adj, d_phiz, d_phix,
          model.d_Vp, cpml.d_sigma_z, cpml.d_sigma_x, nz, nx, dt, dz, dx, nPml, nPad, true);

      res_injection<<<(nrec + 31) / 32, 32>>>(
          d_un_adj, nz, d_res, nSteps - 1, dt, nSteps, nrec,
          src_rec.d_vec_z_rec.at(iShot), src_rec.d_vec_x_rec.at(iShot));

      exchange_wavefield<<<blocks, threads>>>(d_un_adj, d_uc_adj, d_up_adj, nz);

      image_condition<<<blocks, threads>>>(d_uc, d_uc_adj, nz, nx, dz, dx, nPml, model.d_Vp, model.d_VpGrad);
      ...
      for (int it = nSteps - 2; it >= 0; it--) {
        ...
        exchange_wavefield<<<blocks, threads>>>(d_up, d_uc, d_un, nz);
        ...
        propagate<<<blocks, threads>>>(
                d_un, d_uc, d_up, d_phiz, d_phix,
                model.d_Vp, cpml.d_sigma_z, cpml.d_sigma_x, nz, nx, dt, dz, dx, nPml, nPad, false);

        boundaries.field_to_bnd(d_up, it);

        /*
        if (it == 3900 && iShot == 0) {
                 CHECK(cudaMemcpy(h_snap, d_up, nz * nx * sizeof(float), cudaMemcpyDeviceToHost));
                 fileBinWrite(h_snap, nz * nx, "SnapGPU_back_.bin");
                 CHECK(cudaMemcpy(h_snap_adj, d_up_adj, nz * nx * sizeof(float), cudaMemcpyDeviceToHost));
                 fileBinWrite(h_snap_adj, nz * nx, "SnapGPU_adj_.bin");
        ...
        */
        ...
        res_injection<<<(nrec + 31) / 32, 32>>>(
          d_un_adj, nz, d_res, it, dt, nSteps, nrec,
          src_rec.d_vec_z_rec.at(iShot), src_rec.d_vec_x_rec.at(iShot));
        ...
        // if (it == iSnap && iShot == 0) {
        //   CHECK(cudaMemcpy(h_snap_back, d_up, nz * nx * sizeof(float),
        //                    cudaMemcpyDeviceToHost));
        //   CHECK(cudaMemcpy(h_snap_adj, d_up_adj, nz * nx * sizeof(float),
        //                    cudaMemcpyDeviceToHost));
        // }
        /*
        if (it % 10 == 0 && iShot == 0) {
          CHECK(cudaMemcpy(h_snap_adj, d_up_adj, nz * nx * sizeof(float),
           ...
           CHECK(cudaMemcpy(h_snap, d_up, nz * nx * sizeof(float),
                            cudaMemcpyDeviceToHost));
...
#ifdef DEBUG
    fileBinWrite(model.h_VpGrad, nz * nx, "VpGradient.bin");
#endif
  }
