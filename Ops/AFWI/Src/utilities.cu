...
int iRow, jCol;
  ...
  if (idxBnd >= 0 && idxBnd <= nLayerStore * nzBnd - 1) {
    ...
    jCol = (idxBnd - nLayerStore * nzBnd) / nzBnd;
    iRow = (idxBnd - nLayerStore * nzBnd) - jCol * nzBnd;
    d_field((iRow + nPml - 2), (nx - nPml - jCol - 1 + 2)) = d_bnd(idxBnd, indT);
  } else if (idxBnd >= 2 * nLayerStore * nzBnd &&
    idxBnd <= nLayerStore * (2 * nzBnd + nxBnd) - 1) {
    iRow = (idxBnd - 2 * nLayerStore * nzBnd) / nxBnd;
    jCol = (idxBnd - 2 * nLayerStore * nzBnd) - iRow * nxBnd;
...
__global__ void add_source(float *d_un, float amp, int nz,
  ...
  if (isFor) {
    if (gidz >= 0 && gidz < 9 && gidx >= 0 && gidx < 9) {
      int idz = gidz - 9 / 2;
      int idx = gidx - 9 / 2;
      // printf("amp = %f , idz = %d, idx = %d\n", gauss_amp[gidz + gidx * 9], idz, idx);
      d_un[(z_loc + idz) + nz * (x_loc + idx)] +=
...
__global__ void exchange_wavefield(float *d_un, float *d_uc, float *d_up, int nz){
...
__global__ void recording(float *d_uc, int nz, float *d_data,
  ...
  d_data[(iRec) * (nSteps) + (it)] =
      d_uc[d_z_rec[iRec] + d_x_rec[iRec] * nz];
}

__global__ void res_injection(float *d_un_adj, int nz,
  ...
  d_un_adj[d_z_rec[iRec] + nz * d_x_rec[iRec]] +=
