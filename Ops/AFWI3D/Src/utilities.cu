...
__global__ void intialArrayGPU(float *ip, int nz, int nx, int ny, float value) {
  int gidz = threadIdx.x + blockDim.x * blockIdx.x;
  int gidx = threadIdx.y + blockDim.y * blockIdx.y;
  int gidy = threadIdx.z + blockDim.z * blockIdx.z;
  if (gidx < nx && gidy < ny && gidz < nz) {
    int offset = gidz * (nx * ny) +  gidx * ny + gidy;
    ip[offset] = value;
  }
}
...
float compCourantNumber(float *h_Cp, int size, float dt, float dz, float dx) {
  ...
  // Courant_number = max * dt * sqrtf(2.0) / 0.6 / dh_min;
  max_dt = 0.6 * dh_min  / sqrtf(2.0) / max;
  step_radio = ceil(dt / max_dt);
  inner_dt = dt / step_radio;
  return inner_dt;
}
...
void cpmlInit(float *sigma, int N, int nPml, float dh, float f0,
  ...
  const float NPOWER = 2.0;

  thickness_PML = nPml * dh;  // changed here
  CpAve = 5000.0;             // DL make this model independent
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (depth_in_pml >= 0.0) {
      ...
      damp[i] =
          d0_h *  pow(depth_normalized, NPOWER);
    }
...
__global__ void add_source(float *d_un, float amp, int nz, int nx,
  ...
  int gidx = blockIdx.y * blockDim.y + threadIdx.y;
  int gidy = blockIdx.z * blockDim.z + threadIdx.z;
  float scale = pow(100.0, 2);
  if (isFor) {
    if (gidz >= 0 && gidz < 9 && gidx >= 0 && gidx < 9) {
      int idz = gidz - 9 / 2;
      int idx = gidx - 9 / 2;
      int idy = gidy - 9 / 2;
      // printf("idz = %d, idx = %d, idy = %d, amp = %f\n", idz, idx, idy, gauss_amp[gidz + gidx * 9 + gidy * 81]);
      d_un[(z_loc + idz) + nz * (x_loc + idx) + (nz * nx) * (y_loc + idy)] +=
          scale * amp * dt * gauss_amp[gidz + gidx * 9 + gidy * 81];
...
__global__ void recording(float *d_uc, int nz, int nx, float *d_data,
  ...
  d_data[(iRec) * (nSteps) + (it)] =
      d_uc[d_z_rec[iRec] + d_x_rec[iRec] * nz + d_y_rec[iRec] * (nz * nx)];
}
...
__global__ void res_injection(float *d_un_adj, int nz, int nx,
  ...
  d_un_adj[d_z_rec[iRec] + nz * d_x_rec[iRec] + (nz * nx) * d_y_rec[iRec]] +=
...
void bp_filter1d(int nSteps, float dt, int nrec, float *d_data, float *filter) {
  ...
  intial2dArrayGPU<<<blocks, threads>>>(d_data_pad, nSteps_pad, nrec, 0.0);
...
float source_update(int nSteps, float dt, int nrec, float *d_data_obs,
  ...
  intial2dArrayGPU<<<blocks, threads>>>(d_data_obs_pad, nSteps_pad, nrec, 0.0);
  intial2dArrayGPU<<<blocks, threads>>>(d_data_cal_pad, nSteps_pad, nrec, 0.0);
  intial2dArrayGPU<<<(nSteps_pad + 31) / 32, 32>>>(d_source_pad, nSteps_pad, 1,
                                                 0.0);
