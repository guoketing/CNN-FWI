...
__global__ void add_source(float *d_szz, float *d_sxx, float amp, int nz,
  ...
  if (isFor) {
    if (gidz >= 0 && gidz < 9 && gidx >= 0 && gidx < 9) {
      ...
      int idx = gidx - 9 / 2;
      // printf("amp = %f  ", amp);
      d_szz[(z_loc + idz) + nz * (x_loc + idx)] += scale * amp * dt * gauss_amp[gidz + gidx * 9];
      d_sxx[(z_loc + idz) + nz * (x_loc + idx)] += scale * amp * dt * gauss_amp[gidz + gidx * 9];
    } else {
      return;
    }
  } else {
...
__global__ void recording(float *d_szz, float *d_sxx, int nz, float *d_data_z, float *d_data_x,
  ...
  d_data_z[(iRec) * (nSteps) + (it)] = d_szz[d_z_rec[iRec] + d_x_rec[iRec] * nz];
  d_data_x[(iRec) * (nSteps) + (it)] = d_sxx[d_z_rec[iRec] + d_x_rec[iRec] * nz];
}

__global__ void res_injection(float *d_szz_adj, float *d_sxx_adj, int nz,
                              float *d_res_z, float *d_res_x, int it, float dt, int nSteps,
                              int nrec, int *d_z_rec, int *d_x_rec) {
  int iRec = threadIdx.x + blockDim.x * blockIdx.x;
  if (iRec >= nrec) {
    return;
  }
  d_szz_adj[d_z_rec[iRec] + nz * d_x_rec[iRec]] += d_res_z[(iRec) * (nSteps) + (it)];
  d_sxx_adj[d_z_rec[iRec] + nz * d_x_rec[iRec]] += d_res_x[(iRec) * (nSteps) + (it)];
}
...
__global__ void source_grad(float *d_szz_adj, float *d_sxx_adj, int nz,
  ...
  int id = threadIdx.x + blockDim.x * blockIdx.x;
  if (id == 0) {
    d_StfGrad[it] = -(d_szz_adj[z_src + nz * x_src]) * dt;
  } else {
    return;
  }
}
